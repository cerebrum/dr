...
src_install() {
	# desktop entries
	make_desktop_entry "${PN} Game.Mod=cnc Graphics.Renderer=Gl" \
		"OpenRA ver. ${MY_PV} (Gl Renderer)" ${PN} "StrategyGame" \
		"GenericName=OpenRA - Command & Conquer (Gl)" || die
	make_desktop_entry "${PN} Game.Mod=ra Graphics.Renderer=Gl" \
		"OpenRA ver. ${MY_PV} (Gl Renderer)" ${PN} "StrategyGame" \
		"GenericName=OpenRA - Red Alert (Gl)" || die
	make_desktop_entry "${PN} Game.Mod=d2k Graphics.Renderer=Gl" \
		"OpenRA ver. ${MY_PV} (Gl Renderer)" ${PN} "StrategyGame" \
	...
	if use cg ; then
		# cg desktop entries
		make_desktop_entry "${PN} Game.Mod=cnc Graphics.Renderer=Cg" \
			"OpenRA ver. ${MY_PV} (Cg Renderer)" ${PN} "StrategyGame" \
			"GenericName=OpenRA - Command & Conquer (Cg)" || die
		make_desktop_entry "${PN} Game.Mod=ra Graphics.Renderer=Cg" \
			"OpenRA ver. ${MY_PV} (Cg Renderer)" ${PN} "StrategyGame" \
			"GenericName=OpenRA - Red Alert (Cg)" || die
		make_desktop_entry "${PN} Game.Mod=d2k Graphics.Renderer=Cg" \
			"OpenRA ver. ${MY_PV} (Cg Renderer)" ${PN} "StrategyGame" \
