...
src_install() 
{
	...
	for myrenderer in $(usex cg "Cg Gl" "Gl") ; do
		make_desktop_entry "${PN} Game.Mod=cnc Graphics.Renderer=${myrenderer}" \
			"OpenRA ver. ${MY_PV} (${myrenderer} renderer)" ${PN} "StrategyGame" \
			"GenericName=OpenRA - Command & Conquer (${myrenderer})"
		make_desktop_entry "${PN} Game.Mod=ra Graphics.Renderer=${myrenderer}" \
			"OpenRA ver. ${MY_PV} (${myrenderer} renderer)" ${PN} "StrategyGame" \
			"GenericName=OpenRA - Red Alert (${myrenderer})"
		make_desktop_entry "${PN} Game.Mod=d2k Graphics.Renderer=${myrenderer}" \
			"OpenRA ver. ${MY_PV} (${myrenderer} renderer)" ${PN} "StrategyGame" \
